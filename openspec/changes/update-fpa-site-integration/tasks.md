...
- [ ] 0.1 Confirmar el nombre canónico (propuesta: "Uso y costos de IA") y
      definir la clave en `config/fpa.json` (p.ej. `site_name`)
- [ ] 0.2 Traducir la guía al español y committearla como
      `docs/fpa-analyses-guide.md` (fuente de contenido; IDs FPA-xxx intactos)
- [ ] 0.3 **Pase de grounding** afirmación por afirmación contra la
...
- [ ] 0.4 Crear tickets bd del epic (fases 1–5)

## 1. Tema compartido + nombre llano (identidad visual)

- [ ] 1.1 TDD: test de paridad de tokens (los generadores emiten el mismo
      `:root`/reglas desde `scripts/site-theme.py`)
- [ ] 1.2 Extraer el `<style>` compartido de `viz-dashboard.py` a
      `scripts/site-theme.py` (mismos nombres de variables) e interpolarlo en
      `viz-index.py`
- [ ] 1.3 Migrar `viz-fpa.py` al tema compartido: reemplazar paleta crema/carbón,
...
- [ ] 3.6 Regen HTMLs + goldens; suite verde; ro5u; commit

## 4. Consolidación (eliminar insights)

- [ ] 4.1 Eliminar `data/dashboard.html` y `scripts/viz-dashboard.py` del repo
      (git history los preserva); sin referencias restantes en tests/CI/justfile
- [ ] 4.2 `viz-index.py`: Dashboard (nombre llano) como principal + Gantt; sin
      entrada para insights
- [ ] 4.3 README: filas del insights eliminadas, jerarquía documentada,
      `--check-docs` alineado
- [ ] 4.4 Regen `index.html`; suite + ro5u + commit

## 5. Cierre

- [ ] 5.1 Doble corrida de todos los generadores: byte-idénticos salvo timestamp
      (determinismo)
- [ ] 5.2 Suite completa + Playwright verde; `openspec validate --strict`
- [ ] 5.3 Deploy Pages verificado en producción (tema, nombre llano, guía,
      marginalia, índice sin insights)
- [ ] 5.4 Cerrar tickets bd, archivar change, push
